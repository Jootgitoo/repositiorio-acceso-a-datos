...
public class ControladorArchivo implements ActionListener {
  ...
  public void actionPerformed (ActionEvent evento) {
    switch (evento.getActionCommand()) {
      ...
      case InterfazVista.RENOMBRAR_ARCHIVO_EXISTENTE_CLASE_NIO -> {
          this.nombre = this.vista.getNombreBase();
          String nuevoNombre = this.vista.getNombre();
          this.ruta = this.vista.getRuta();
          
          this.modelo.renombrarArchivoNIO(ruta, nombre, nuevoNombre);
      }
      case InterfazVista.COPIAR_ARCHIVO_CLASE_NIO -> {
            this.ruta = this.vista.getRuta();
            String nuevaRuta = this.vista.getNuevaRuta();
            String nombre = this.vista.getNombre();
          
            this.modelo.copiarArchivo(ruta, nuevaRuta, nombre);
            this.vista.operacionExitosa();
            this.vista.limpiarCampos();
      }
      case InterfazVista.MOVER_ARCHIVO_CLASE_NIO -> {
            this.ruta = this.vista.getRuta();
            String nuevaRuta = this.vista.getNuevaRuta();
            String nombre = this.vista.getNombre();

            this.modelo.moverArchivoNIO(ruta, nuevaRuta, nombre);
            this.vista.operacionExitosa();
            this.vista.limpiarCampos();
        }
